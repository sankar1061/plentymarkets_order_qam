...
class NovalnetEpsPaymentMethod extends PaymentMethodService
{
    ...
    {
        return (bool)(($this->configRepository->get('Novalnet.novalnet_eps_payment_active') == 'true') && is_numeric($this->paymentHelper->getNovalnetConfig('novalnet_vendor_id')) && !empty($this->paymentHelper->getNovalnetConfig('novalnet_auth_code')) && is_numeric($this->paymentHelper->getNovalnetConfig('novalnet_product_id')) && is_numeric($this->paymentHelper->getNovalnetConfig('novalnet_tariff_id')) && !empty($this->paymentHelper->getNovalnetConfig('novalnet_access_key')));
    }
    ...
    {   
        if(empty($name = trim($this->configRepository->get('Novalnet.novalnet_eps_payment_name'))))
        {
            $name = $this->paymentHelper->getTranslatedText('eps_name');
        }
        return $name;
    }
    ...
    {
        if(empty($description = trim($this->configRepository->get('Novalnet.novalnet_eps_description'))))
        {
